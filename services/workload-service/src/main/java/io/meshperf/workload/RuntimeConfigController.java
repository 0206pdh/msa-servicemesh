...
@RestController @RequestMapping("/internal/v1") class RuntimeConfigController {
    ...
    RuntimeConfigController(Environment env){var v=new TreeMap<String,String>(); v.put("service",env.getRequiredProperty("spring.application.name")); v.put("role",env.getProperty("mesh-perf.workload-role","target-a")); v.put("javaVersion",Runtime.version().toString()); v.put("imageDigest",env.getProperty("mesh-perf.image-digest","local")); v.put("tracingSamplingProbability",env.getProperty("management.tracing.sampling.probability","1.0")); v.put("topologyTarget","none"); v.put("fingerprint",sha(v.toString())); snapshot=Map.copyOf(v);}
